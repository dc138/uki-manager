...
#[proc_macro_derive(ParseTemplate, attributes(template))]
pub fn parse_template(item: pm::TokenStream) -> pm::TokenStream {
    ...
    let template_struct_ident = input
        .attrs
        .into_iter()
        .find_map(|attr| {
        ...
        .expect("template struct attribute not found");

    let impl_field_init = fields.into_iter().map(|field| {
        let ident = field.ident.expect("all fields should have identifiers");

        quote::quote! {
            #ident: self.#ident.parse_template(template)
        }
    });

    pm::TokenStream::from(quote::quote! {
        impl crate::traits::ParseTemplate<#template_struct_ident> for #ident {
            fn parse_template(&self, template: &#template_struct_ident) -> Self {
                Self {
                    #(#impl_field_init,)*
                }
            }
